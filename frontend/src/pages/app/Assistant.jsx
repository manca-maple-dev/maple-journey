    ...
    <div className="flex h-full flex-col bg-background" data-testid="assistant-page">
      ...
      <div className="shrink-0 border-b border-border bg-background/80 backdrop-blur">
        <div className="mx-auto w-full max-w-3xl px-4 pt-3 sm:px-6">
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-maple text-lg text-white">🍁</div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <h1 className="font-display text-base font-bold tracking-tight sm:text-lg">Ask Maple</h1>
                {usage && (
                  <span data-testid="assistant-tier-badge" className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${isPaid ? "bg-brand-50 text-brand-600 dark:bg-brand-500/10" : "bg-secondary text-muted-foreground"}`}>
                    {isPaid ? <InfinityIcon className="h-3 w-3" /> : null}
                    {usage.tier === "free" ? "Free" : usage.tier === "plus" ? "Plus" : "Family"}
                  </span>
                )}
              </div>
              <p className="truncate text-xs text-muted-foreground">Cited answers from IRCC, the CRA &amp; Service Canada.</p>
            ...
            {messages.length > 0 && (
              <div className="flex items-center gap-1.5">
                <button onClick={askFromPageContext} data-testid="assistant-context-ask" className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground">
                  <Sparkles className="h-3.5 w-3.5" /> <span className="hidden sm:inline">Use my page context</span>
                </button>
                <button onClick={resetChat} data-testid="assistant-new-chat" className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground">
          ...
          {usage && (
            <div data-testid="assistant-retention" className={`mb-2 mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg px-2.5 py-1.5 text-[11px] ${isPaid ? "text-brand-700 dark:text-brand-300" : "bg-amber-50 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300"}`}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="assistant-scroll">
        <div className="mx-auto w-full max-w-3xl px-4 sm:px-6">
          {messages.length === 0 ? (
            <div className="flex min-h-[55vh] items-center justify-center py-8 text-center">
              <div className="max-w-md">
                <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-brand-50 text-brand-600 dark:bg-brand-500/10"><MessageSquareText className="h-7 w-7" /></div>
                <h2 className="mt-5 font-display text-lg font-semibold">What would you like to know?</h2>
                <p className="mt-2 text-sm text-muted-foreground">Ask about permits, your SIN, health coverage, taxes, benefits or finding legal help. Every answer is cited.</p>
                <div className="mt-6 grid gap-2 sm:grid-cols-2">
                  {SUGGESTIONS.map((s) => (
                    <button key={s} onClick={() => submit(s)} data-testid="assistant-suggestion" className="rounded-xl border border-border bg-card px-3 py-2.5 text-left text-sm transition-colors hover:border-brand-500 hover:text-brand-500">{s}</button>
                  ))}
                </div>
              </div>
            </div>
              ...
              {messages.map((m, i) => {
                ...
                return (
                  <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`} data-testid={`chat-msg-${m.role}`}>
                    ...
                    <div className={`max-w-[84%] ${m.role === "user" ? "" : "w-full sm:max-w-2xl"}`}>
                      <div className={`whitespace-pre-wrap rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${m.role === "user" ? "rounded-tr-sm bg-brand-500 text-white" : "rounded-tl-sm bg-secondary text-foreground"}`}>
                        {cleanContent
                          ? (m.role === "assistant"
          ...
          {limitReached ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); submit(); }} className="flex items-end gap-2">
              <textarea
                ...
                className="max-h-40 min-h-[48px] flex-1 resize-none rounded-2xl border border-border bg-card px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-brand-500/40"
              />
              <button type="submit" disabled={sending || !input.trim()} data-testid="assistant-send" className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-brand-500 text-white transition-transform hover:-translate-y-0.5 disabled:opacity-50"><Send className="h-5 w-5" /></button>
            </form>
          )}
